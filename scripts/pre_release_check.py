...
def parse_release_tag(tag: str, base_version: str) -> int:
    ...
    if not match:
        fail(f"Tag must match {base_version}.<release>-vcodex-PLUS, got {tag!r}")
...
def main() -> int:
    ...
    parser.add_argument("--tag", required=True, help="Candidate stable tag, e.g. 1.2.0.39-vcodex-PLUS")
    parser.add_argument("--jobs", type=int, default=1, help="PlatformIO build jobs (default: 1)")
    parser.add_argument("--flash-budget-percent", type=float, default=98.5)
